...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  Node* current = head_;
  while(current != tail_ && current->next->data < newData) {
    current = current->next;
  }

  if (current == head_ && head_->data >= newData) {  // newData is less then or equal to than head node
    pushFront(newData);
  } else if (current == tail_ && tail_->data < newData) {  // newData is greater than tail node
    pushBack(newData);
  } else {  // newData is inserted in middle of list
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // Check for emptiness in each list
  if (left.size_ == 0 && right.size_ == 0) {
    return merged;
  } else if (left.size_ == 0 && right.size_ > 0) {
    return right;
  } else if (left.size_ > 0 && right.size_ == 0) {
    return left;
  }
  ...
  bool side = true; // left = true and right = false;
  T left_data = left.front();
  T right_data = right.front();

  while (left.size_ > 0 || right.size_ > 0) {
    if ((right.size_ > 0 && left_data >= right_data)  // If right data is less than left data
        || (left.size_ == 0 && right.size_ > 0)) {  // If right has data and left is empty
      merged.pushBack(right_data);
      right.popFront();
      side = false;
    } else {  // If left data is less than right data or left had data and right does not
      merged.pushBack(left_data);
      left.popFront();
      side = true;
    }

    // Pop element from each list depending on which side the pointer is incremented on
    if (side) {
      if (left.size_ > 0) {
        left_data = left.front();
      }
    } else {
      if (right.size_ > 0) {
        right_data = right.front();
      }
    }
  }
