...
/*int computeHeightHelper(Node *n) {
  if (n == nullptr) {
    return 0;
  } else {
    return 1 + std::max(computeHeightHelper(n->left), computeHeightHelper(n->right));
  }
}
...
void computeHeight(Node *n) {
  if (n == nullptr) {
    ...
    n->height = computeHeightHelper(n);
    // n->height = std::max(computeHeightHelper(n->left), computeHeightHelper(n->right));
